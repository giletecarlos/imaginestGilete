<?php
    ...
    function solicitarCambiarContrasena($email)
    {
        ...
        try{
            ...
            $sql = "UPDATE users SET resetPass = ?, resetPassCode = ? WHERE mail=?";
    ...
    function actualizarContraseña($pass,$mail)
    {
        require('connecta_db_persistent.php');
        try{
            $sql = "UPDATE users SET passHash = ?, resetPassCode = ?, resetPass = ? WHERE mail=?";
            $preparadaActPass = $db->prepare($sql);
            $preparadaActPass->execute(array($pass,null,0,$mail));
